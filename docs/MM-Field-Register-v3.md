...
| `cellNumberTwo` | string | Account | N (not yet displayed publicly — display mechanism not built, see Master Context 21 July session note) | N | N | **NEW FIELD, 18 July; built 21 July.** Secondary number, Channel 3 only. Never used for OTP/token (rare re-registration edge case not policed or tested for). Toggle field name confirmed as `showCellNumberTwo` (matches the `showWhatsapp` convention). Public reveal mechanism not yet built. |
| `showCellNumberTwo` | boolean | Account | N/A (toggle) | N | N | Controls whether the secondary number shows on the public profile once the reveal mechanism exists. Mirrors `showWhatsapp`. Not used for Channel 2 (platform notifications). |
